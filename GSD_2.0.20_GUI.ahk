; GSD Processor v2.0.20 — GUI Edition
; AutoHotkey v1 — compiled with Ahk2Exe
; -------------------------------------------------------
#NoEnv
#Warn
...
global COUNT             := ""
global g_Cancel          := 0
global g_Processed       := 0
global g_Errors          := 0
global g_Total           := 0

; ── Load saved settings ─────────────────────────────────
IniRead, saved_folder,  %A_ScriptDir%\GSD_Settings.ini, Settings, LastFolder,  %A_Space%
IniRead, saved_library, %A_ScriptDir%\GSD_Settings.ini, Settings, LibraryPath, %GSD_LIBRARY_LOC%
if (saved_library != "")
    GSD_LIBRARY_LOC := saved_library

; ── Build GUI ───────────────────────────────────────────
Gui, Main:New, +Resize +MinSize560x480, GSD Processor v2.0.20
Gui, Main:Font, s10, Segoe UI
Gui, Main:Color, 1e1e2e

; Header
Gui, Main:Font, s13 bold, Segoe UI
Gui, Main:Add, Text, x16 y14 cf5c542, GSD Processor
Gui, Main:Font, s9 norm, Segoe UI
Gui, Main:Add, Text, x148 y20 ca0aec0, v2.0.20  ·  NC1 File Processor for Structural Steel
Gui, Main:Add, Text, x16 y38 w540 h1 +0x10 cSilver,  ; separator line

; Folder row
Gui, Main:Font, s9 norm, Segoe UI
Gui, Main:Add, Text,    x16  y52 w100 h20 ce2e8f0, Input Folder:
Gui, Main:Add, Edit,    x120 y50 w390 h22 vInputFolder +ReadOnly cTerminalGrey Background2a2a3e, %saved_folder%
Gui, Main:Add, Button,  x518 y49 w48 h24 gBrowseInput, Browse

; Library row
Gui, Main:Add, Text,    x16  y82 w100 h20 ce2e8f0, Library Path:
Gui, Main:Add, Edit,    x120 y80 w390 h22 vLibraryPath cTerminalGrey Background2a2a3e, %GSD_LIBRARY_LOC%
Gui, Main:Add, Button,  x518 y79 w48 h24 gBrowseLibrary, Browse
Gui, Main:Add, Text,    x16  y108 w540 h1 +0x10 cSilver,

; Progress row
Gui, Main:Add, Text,    x16  y118 w100 h20 ce2e8f0, Progress:
Gui, Main:Add, Progress, x120 y120 w390 h18 vProgressBar cGreen Background2a2a3e +Smooth, 0
Gui, Main:Add, Text,    x518 y118 w50 h20 vProgressLabel ca0aec0 Right, 0 / 0
Gui, Main:Add, Text,    x16  y143 w540 h1 +0x10 cSilver,

; Log area
Gui, Main:Add, Text,    x16  y152 w60 h20 ce2e8f0, Log:
Gui, Main:Add, Button,  x480 y148 w86 h22 gClearLog, Clear Log
Gui, Main:Add, Edit,    x16  y172 w550 h210 vLogBox +ReadOnly +MultiLine -HScroll +WantReturn Background111122 c9b59f5,
Gui, Main:Add, Text,    x16  y388 w540 h1 +0x10 cSilver,

; Stats row
Gui, Main:Add, Text,    x16  y397 w90  h20 vStatProcessed ce2e8f0, Processed: 0
Gui, Main:Add, Text,    x130 y397 w80  h20 vStatErrors   cff4040,  Errors: 0
Gui, Main:Add, Text,    x230 y397 w120 h20 vStatElapsed  ca0aec0,  Elapsed: —

; Buttons
Gui, Main:Add, Button,  x16  y424 w120 h32 gStartProcess Default, ▶  START
Gui, Main:Add, Button,  x148 y424 w100 h32 gCancelProcess, ■  CANCEL
Gui, Main:Add, Button,  x450 y424 w116 h32 gOpenOutputFolder, Open Output ↗

Gui, Main:Show, w580 h472
return

; ── Button handlers ─────────────────────────────────────

BrowseInput:
    FileSelectFolder, chosen, P:\, 3, Select the folder containing NC1 files
    if (chosen != "")
    {
        GuiControl, Main:, InputFolder, %chosen%
        IniWrite, %chosen%, %A_ScriptDir%\GSD_Settings.ini, Settings, LastFolder
    }
return

BrowseLibrary:
    Gui, Main:Submit, NoHide
    FileSelectFolder, chosen, %LibraryPath%, 3, Select the GSD Shapes Library folder
    if (chosen != "")
    {
        GuiControl, Main:, LibraryPath, %chosen%
        IniWrite, %chosen%, %A_ScriptDir%\GSD_Settings.ini, Settings, LibraryPath
    }
return
...
CancelProcess:
    g_Cancel := 1
    AppendLog("⚠ Cancel requested — stopping after current file...")
return

OpenOutputFolder:
    Gui, Main:Submit, NoHide
    if (InputFolder != "") and InStr(FileExist(InputFolder "\Completed"), "D")
        Run, explorer.exe "%InputFolder%\Completed"
    else if (InputFolder != "")
        Run, explorer.exe "%InputFolder%"
return

StartProcess:
    Gui, Main:Submit, NoHide
    FOLDERNAME := InputFolder
    GSD_LIBRARY_LOC := LibraryPath
    if (LibraryPath != "")
        IniWrite, %LibraryPath%, %A_ScriptDir%\GSD_Settings.ini, Settings, LibraryPath

    if (FOLDERNAME = "")
    {
        MsgBox, 48, No Folder Selected, Please select an input folder first.
        return
    }
    if !FileExist(FOLDERNAME)
    {
        MsgBox, 48, Folder Not Found, The selected folder does not exist:`n%FOLDERNAME%
        return
    }
    ...
    if (g_Total = 0)
    {
        MsgBox, 48, No NC1 Files, No .nc1 files found in:`n%FOLDERNAME%
        return
    }
    ...
    GuiControl, Main:, ProgressLabel, 0 / %g_Total%
    GuiControl, Main:, StatProcessed, Processed: 0
    GuiControl, Main:, StatErrors,    Errors: 0
    GuiControl, Main:, StatElapsed,   Elapsed: ...
    GuiControl, Main:, LogBox,

    AppendLog("── Starting GSD run ──────────────────────────────")
    AppendLog("Folder : " FOLDERNAME)
    AppendLog("Library: " GSD_LIBRARY_LOC)
    AppendLog("Files  : " g_Total " NC1 files found")
    AppendLog("")

    StartTime := A_TickCount
    GuiControl, Main:Disable, StartProcess
    ...
    Loop, Files, %FOLDERNAME%\*.nc1
    {
        ...
        AppendLog("Processing  " A_LoopFileName " ...")
        try
        {
            ReadFile()
            CheckOffset()
            ...
            AddOrRemoveScribe()
            SaveNewFileContents()
            AppendLog("  ✓  " PART_TYPE " · " PART_GRADE " · Machine: " MACHINE_TYPE)
            g_Processed++
        }
        catch e
        {
        ...
        pct := Round((g_Processed + g_Errors) / g_Total * 100)
        done := g_Processed + g_Errors
        GuiControl, Main:, ProgressBar, %pct%
        GuiControl, Main:, ProgressLabel, %done% / %g_Total%
        GuiControl, Main:, StatProcessed, Processed: %g_Processed%
        GuiControl, Main:, StatErrors,    Errors: %g_Errors%
        ; Let GUI breathe
        Sleep, 10
    }

    EndTime    := A_TickCount
    elapsed    := Round((EndTime - StartTime) / 1000.0, 1)
    GuiControl, Main:, StatElapsed, Elapsed: %elapsed%s
    GuiControl, Main:Enable, StartProcess

    AppendLog("")
    AppendLog("── Complete ──────────────────────────────────────")
    AppendLog("Processed : " g_Processed "   Errors: " g_Errors "   Time: " elapsed "s")
return

; ── Helper: append a line to the log box ────────────────
AppendLog(msg)
{
    GuiControlGet, current,, LogBox
    if (current = "")
        GuiControl, Main:, LogBox, %msg%
    else
    {
        new := current "`n" msg
        GuiControl, Main:, LogBox, %new%
    }
    ; Auto-scroll to bottom
    SendMessage, 0x115, 7, 0, Edit1, GSD Processor v2.0.20
    return
}
...
; ════════════════════════════════════════════════════════
;  ORIGINAL GSD LOGIC — unchanged from GSD_2.0.20.exe
; ════════════════════════════════════════════════════════
...
FixGrade()
{
    grade_line := OFFSET
    grade_line += 6
    for _, old_grade in ["300W", "316SS", "SS-316"]
    {
        if instr(LINE_ARRAY[grade_line], old_grade)
        {
            JOKER := ReadLine(6)
            SpaceCount()
            JOKER := ""
...
New_Scribe()
{
    ...
    IfLess PART_LENGTH, 152.4
        line2 := "  o   25.40o  " . BeamCenter . "`t0.00  13  " . PART_NAME
    else IfLess PART_LENGTH, 304.8
        line2 := "  o   25.40o  " . BeamCenter . "`t0.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    else IfLess PART_LENGTH, 6096
        line2 := "  o   50.80o  " . BeamCenter . "`t0.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    else
        line2 := "  o   914.40o " . BeamCenter . "`t0.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
...
New_Angle_Scribe()
{
    ...
    line1 := "SI"
    IfLess PART_LENGTH, 152.4
        line2 := "  h   " . ScribeStart . "u  " . BeamCenter . "`t180.00  13  " . PART_NAME
    else IfLess PART_LENGTH, 304.8
        line2 := "  h   " . ScribeStart . "u  " . BeamCenter . "`t180.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    else
        line2 := "  h   " . ScribeStart . "u  " . BeamCenter . "`t180.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    StringReplace, line2, line2, _, -, All
    LINE_ARRAY.Push(line1)
...
Plate_Scribe()
{
    ...
    line1 := "SI"
    IfLess PART_LENGTH, 152.4
        line2 := "  v   25.40o  " . BeamCenter . "`t0.00  13  " . PART_NAME
    else IfLess PART_LENGTH, 304.8
        line2 := "  v   25.40o  " . BeamCenter . "`t0.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    else IfLess PART_LENGTH, 6096
        line2 := "  v   50.80o  " . BeamCenter . "`t0.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    else
        line2 := "  v   914.40o " . BeamCenter . "`t0.00  13  " . PROJECT_NUMBER . "-" . PART_NAME
    StringReplace, line2, line2, _, -, All
    LINE_ARRAY.Push(line1)
    LINE_ARRAY.Push(line2)
}
...
ShapesLibrary()
{
    if ((MACHINE_TYPE = "Beamline") or (MACHINE_TYPE = "Anglemaster") or (PART_TYPE = "Plate")) and ((PART_TYPE != "WT") and (PART_TYPE != "Bar") and (PART_TYPE != "Other"))
    {
        ...
        {
            variables := [8, 9, 11, 12, 13, 14, 15, 16]
            for index, variable in variables
            {
